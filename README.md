...
貼上任意購物網站網址，實際讀取網頁內容，交給 Claude API 解析成優惠清單、跨站比價，並支援 AI 對話查詢。

## 這個專案怎麼運作

1. 前端把你貼上的每個網址，各自送到 `/api/analyze`
2. 後端用 `fetch` 抓取該網址的原始 HTML，清理成純文字後，呼叫 Claude API 做「結構化擷取」——只允許擷取網頁中真的出現的資料，找不到就回傳空陣列，不會編造
3. 「同規格商品比價」用商品名稱字串比對做自動分組（任意網站商品命名不同，這是務實但誠實的做法，不保證 100% 精準）
4. 「AI 對話」也是呼叫 Claude API，但嚴格限制只能根據已分析到的網站資料回答，找不到就照你原本規則回覆「目前提供的網站中沒有找到相關資訊，請補充更詳細內容或新增網站」
...
# 編輯 .env.local，填入你自己的 Anthropic API 金鑰
npm run dev
```

打開 http://localhost:3000

金鑰申請位置：https://console.anthropic.com/settings/keys
...
- 使用的模型是 `claude-haiku-4-5-20251001`（速度快、成本低），如果解析品質不如預期，可以在 `lib/anthropicClient.js` 把 `EXTRACTION_MODEL` 換成更強的模型（例如 `claude-sonnet-4-6`），但費用會較高。
...
pages/
  ...
  api/analyze.js     抓取單一網址並用 AI 擷取優惠資訊
  api/chat.js         AI 對話問答，只根據已分析資料回答
lib/
  extractText.js      HTML 轉純文字工具
  anthropicClient.js  Anthropic API client 共用模組
```
